...
CAPS_IMPORT = '\t"github.com/Izaiaspertrelly/openfoundry/libs/capabilities"\n'
...
def patch(path: pathlib.Path, knob: dict) -> tuple[str, str]:
    ...
    if "libs/auth-middleware\"" in src:
        anchor = re.search(r'^(\tauthmw "github\.com/Izaiaspertrelly/openfoundry/libs/auth-middleware"\n)', src, re.M)
    else:
        anchor = re.search(r'^(\t"github\.com/Izaiaspertrelly/openfoundry/libs/core-models/health"\n)', src, re.M)
    if not anchor:
        # Fallback: just before observability.
        anchor = re.search(r'^(\t"github\.com/Izaiaspertrelly/openfoundry/libs/observability"\n)', src, re.M)
    if not anchor:
