// Package oracle provides the Go adapter for Oracle Database catalogs.
//
// Rust's `services/connector-management-service/src/connectors/oracle.rs` is
// still an empty placeholder, so there is no upstream native Oracle driver
// implementation to port. To avoid leaving discovery and preview unusable for
// Oracle connections, this adapter follows the same tabular catalog bridge
...
// Rust-equivalent streaming/ingestion contract exists for Oracle; those
...
import (
	...
	"github.com/Izaiaspertrelly/openfoundry/services/connector-management-service/internal/adapters"
	"github.com/Izaiaspertrelly/openfoundry/services/connector-management-service/internal/adapters/catalogbridge"
	"github.com/Izaiaspertrelly/openfoundry/services/connector-management-service/internal/models"
)

const (
	// ConnectorType is the `connections.connector_type` value the registry
	// binds this adapter under, matching Rust's `connectors::oracle` module name.
	ConnectorType = "oracle"

	defaultSourceKind = "oracle_table"
)

// identityFields are the config keys required when Oracle is configured via a
// `resource_path_template` instead of an inline `tables` catalog.
var identityFields = []string{"host"}

// Adapter is the [adapters.ConnectorAdapter] implementation for Oracle.
type Adapter struct {
	bridge *catalogbridge.Bridge
}

// New returns an Oracle [Adapter] backed by [http.DefaultClient].
func New() *Adapter {
	return &Adapter{bridge: catalogbridge.New(ConnectorType, defaultSourceKind, identityFields)}
}

// Factory returns an [adapters.Factory] that constructs fresh Oracle adapters.
...
// DiscoverSources returns inline Oracle tables or remote bridge catalog entries
// using the shared tabular bridge.
...
// QueryVirtualTable returns inline sample rows or remote bridge rows for an
// Oracle selector using the shared tabular bridge.
...
// nor the shared tabular bridge exposes Oracle Arrow IPC streaming yet.
...
// BuildIngestSpec returns [adapters.ErrNotImplemented]: Oracle is not wired into
// the Rust ingestion bridge yet.
