// Package graphql is the placeholder Go side of
// `services/connector-management-service/src/connectors/graphql.rs`.
//
...
//
// When Rust grows a real GraphQL connector, the file-by-file port lands
// here behind the same [ConnectorType] string; callers that resolved
// through the registry switch over without having to re-register.
package graphql
...
// binds this adapter under, matching Rust's `connectors::graphql` module name.
const ConnectorType = "graphql"

// Adapter is the skeleton [adapters.ConnectorAdapter] for GraphQL. It
...
// `connectors::graphql` module is an empty placeholder.
