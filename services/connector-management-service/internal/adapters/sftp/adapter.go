// Package sftp is the placeholder Go side of
// `services/connector-management-service/src/connectors/sftp.rs`.
//
...
//
// When Rust grows a real SFTP connector, the file-by-file port lands here
// behind the same [ConnectorType] string; callers that resolved through
// the registry switch over without having to re-register.
package sftp
...
// binds this adapter under, matching Rust's `connectors::sftp` module name.
const ConnectorType = "sftp"

// Adapter is the skeleton [adapters.ConnectorAdapter] for SFTP. It holds
...
// `connectors::sftp` module is an empty placeholder.
