...
func New(cfg *config.Config, jwt *authmw.JWTConfig, h *handlers.Handlers, m *observability.Metrics, sm StreamingMetadata, probes ...capabilities.DependencyProbe) *http.Server {
	r := chi.NewRouter()
	...
	if _, err := caps.IngestChiRoutes(r, capabilities.IngestOptions{
		IDPrefix:  "ingestion-replication",
		AuthPaths: []string{"/api/v1"},
		Tags:      []string{"ingestion"},
	}); err != nil {
		panic("ingestion-replication-service: capability ingest failed: " + err.Error())
	}
