...
```sh
# default = dev environment, repo = github.com/Izaiaspertrelly/openfoundry, revision = main
...
```sh
OPENFOUNDRY_GITOPS_REPO=https://github.com/Izaiaspertrelly/openfoundry.git \
...
A single repository URL is used throughout the Argo CD manifests. Override
`OPENFOUNDRY_GITOPS_REPO` when deploying from a mirror.
covers them all.
