...
```sh
pnpm --filter @izaiaspertrelly/web dev     # vite dev server
pnpm --filter @izaiaspertrelly/web build   # tsc -b && vite build
pnpm --filter @izaiaspertrelly/web check   # tsc -b --noEmit (typecheck only)
pnpm --filter @izaiaspertrelly/web lint    # eslint .
pnpm --filter @izaiaspertrelly/web test    # vitest run (unit)
pnpm --filter @izaiaspertrelly/web test:e2e  # playwright (needs the backend running)
```
